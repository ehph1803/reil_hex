...
class hexPosition(object):
    ...
    winner : int
        Whether the game is won and by whom. '0' means 'no winner'. '1' means 'white' has won. '-1'
        means 'black' has won.
    ...
    def human_vs_machine(self, human_player=1, machine=None):
        """
        Play a game against an AI. The variable machine must point to a function that maps a board state
        and an action set to an element of the action set.
    ...
    def recode_black_as_white(self, verbose=False, invert_colors=True):
        """
    ...
    def recode_coordinates(self, coordinates):
        ...
        """
        assert (0 <= coordinates[0] <= self.size - 1), \
            "There is something wrong with the first coordinate."

        assert (0 <= coordinates[1] <= self.size - 1), \
    ...
    def scalar_to_coordinates(self, scalar):
        ...
        assert (0 <= coord1 <= self.size - 1), "The scalar input is invalid."
        assert (0 <= coord2 <= self.size - 1), "The scalar input is invalid."
    ...
    def machine_vs_machine(self, machine1=None, machine2=None):
        ...
        The variables machine1 and machine2 must point to a function that maps a board state and
        an action set to an element of the action set.
